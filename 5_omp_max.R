...
m4_max_l_dat <- omp_dat %>% group_by(bay, location_clean, f_year) %>% # group by location and year
...
# Emma's paths
save(m4_max_temp_time_sal, file = "~/Dropbox/_Projects/PEI Oysters/Model_Fits/OMP/m4_max_temp_time_sal.Rdata")
load("~/Dropbox/_Projects/PEI Oysters/Model_Fits/OMP/m4_max_temp_time_sal.Rdata")

# Maddy's path
save(max_temp_time_sal, file = "~/Data/Model_fits/OMP/m4_max_temp_time_sal.Rdata")
load("~/Data/Model_fits/OMP/m4_max_temp_time_sal.Rdata")

# sanity checks 
summary(m4_max_temp_time_sal)
conditional_effects(m4_max_temp_time_sal)

pp_check(m4_max_temp_time_sal)

color_scheme_set("darkgray")
fig_pp_max4 <- pp_check(m4_max_temp_time_sal) +   
...
# ============================================================
# FIGURE WORKFLOW FOR m4_max_temp_time_sal
# ============================================================
...
m4_max_sal_sd <- sd(m4_max_l_dat$salinity.m, na.rm = TRUE)

m4_max_sal_df <- tibble(
  salinity.m = c(-m4_max_sal_sd, 0, m4_max_sal_sd),
  sal_label  = factor(
...
m4_max_year_levels <- m4_max_l_dat %>%
  distinct(n_year) %>%
  arrange(n_year) %>%
  pull(n_year)
...
m4_max_pred_grid <- m4_max_l_dat %>%
  distinct(n_year, water_temp) %>%      # observed combinations only
  crossing(m4_max_sal_df) %>%
  mutate(
    # Center predictors exactly as used in the model
    water_temp.m = water_temp - mean(m4_max_l_dat$water_temp, na.rm = TRUE),
    n_year.m     = n_year     - mean(m4_max_l_dat$n_year,     na.rm = TRUE),
...
m4_max_ep <- posterior_epred(
  m4_max_temp_time_sal,
  newdata    = m4_max_pred_grid,
  re_formula = NA
)
...
m4_max_ep_long <- as_tibble(m4_max_ep) %>%
...
m4_max_summ_mid <- m4_max_summ %>%
  filter(sal_label == "Mean salinity")

m4_max_fig1_mid_max <- ggplot(
  m4_max_summ_mid, aes(x = water_temp, y = estimate, group = year_group, colour = year_group)) +
  geom_ribbon(aes(ymin = lower90, ymax = upper90, fill = year_group), alpha = 0.12, colour = NA) +
  geom_line(linewidth = 0.7) +
  scale_colour_viridis_d(option = "viridis", name = "Monitoring year") +
  scale_fill_viridis_d(option = "viridis", name = "Monitoring year") +
...
# Figure caption: For mean salinity conditions, the model shows how the predicted timing of peak larval
# abundance varies with surface water temperature (phenology) and how this relationship
# differs among monitoring years.
# AKA (showing when the yearly peak in larvae is expected to happen at diff temps and how that timing differs
# from year to year when salinity is constant)
# Salinity is fixed at the mean (0 SD) = typical (standardized) salinity conditions relative to each bay
# Shaded ribbons represent 90% Bayesian credible intervals around the mean predicted date of peak larval abundance
# (>250 μm)
# (90% ribbon = 5th to 95th pct, lines = mean predicted date)
...
# #   m4_max_ep_long has columns: .draw, row_id, epred, water_temp, year_group, sal_label, ...
# #
# # Here we keep only the mean-salinity panel, because we want spaghetti
# # for the main “mean salinity” story figure.
# # ------------------------------------------------------------
# m4_max_ep_mid_long <- m4_max_ep_long %>%
#   filter(sal_label == "Mean salinity")
# 
# # ------------------------------------------------------------
...
# # ------------------------------------------------------------
# m4_max_ci_pool_size <- min(2000, n_distinct(m4_max_ep_mid_long$.draw))
# 
# m4_max_ci_pool_ids <- m4_max_ep_mid_long %>%
#   distinct(.draw) %>%
#   slice_sample(n = m4_max_ci_pool_size) %>%  # randomly choose draw IDs
#   pull(.draw)
# 
# # Filter the mean-salinity posterior predictions down to only those draws
# m4_max_ep_ci_long <- m4_max_ep_mid_long %>%
#   filter(.draw %in% m4_max_ci_pool_ids)
# 
# # ------------------------------------------------------------
...
# # ------------------------------------------------------------
# m4_max_bounds80 <- m4_max_ep_ci_long %>%
#   group_by(row_id) %>%
#   summarise(
...
# # ------------------------------------------------------------
# m4_max_draw_keep <- m4_max_ep_ci_long %>%
#   left_join(m4_max_bounds80, by = "row_id") %>%
#   mutate(
...
# # ------------------------------------------------------------
# 
# m4_max_n_spaghetti <- 150  # <-- add this
# 
# # how many draws are actually available to sample from?
# m4_max_n_keep <- min(m4_max_n_spaghetti, nrow(m4_max_draw_keep))
# 
# m4_max_spaghetti_draw_ids <- m4_max_draw_keep %>%
#   slice_sample(n = m4_max_n_keep) %>%
#   pull(.draw)
# 
# 
# 
# # Pull the long epred values for only those spaghetti draws
# m4_max_ep_spag_long <- m4_max_ep_ci_long %>%
#   filter(.draw %in% m4_max_spaghetti_draw_ids)
# 
# # ------------------------------------------------------------
...
# #   - thicker posterior mean line per year (from m4_max_summ_mid)
# #
# # Note:
...
# # ------------------------------------------------------------
# m4_max_fig1_spag <- ggplot() +
#   geom_line(
#     data = m4_max_ep_spag_long,
#     aes(
#       x = water_temp,
#       y = epred,
...
#   used to fit the model (m4_max_l_dat). No artificial temp sequences.
#
# UNCERTAINTY:
...
# ------------------------------------------------------------
m4_max_year_sal_temp_grid <- m4_max_l_dat %>%
  distinct(n_year, water_temp) %>%      # <-- observed year-temp combos only
  crossing(m4_max_sal_df) %>%           # <-- add salinity slices
  mutate(
...
# ------------------------------------------------------------
m4_max_ep2 <- posterior_epred(
  m4_max_temp_time_sal,
  newdata    = m4_max_year_sal_temp_grid,
  re_formula = NA
)
...
# ------------------------------------------------------------
m4_max_ep2_long <- as_tibble(m4_max_ep2) %>%
  ...
  mutate(row_id = readr::parse_number(row_id)) %>%
  left_join(
    m4_max_year_sal_temp_grid %>% select(row_id, n_year, sal_label, water_temp),
    by = "row_id"
  )
...
# ------------------------------------------------------------
m4_max_draw_means <- m4_max_ep2_long %>%
...
# ------------------------------------------------------------
m4_max_int_summ <- m4_max_draw_means %>%
...
View(m4_max_int_summ)

# ------------------------------------------------------------
# 6f) Figure: 3 salinity panels
# ------------------------------------------------------------
m4_max_fig_intercepts_3panel <- ggplot(
  m4_max_int_summ, aes(x = n_year, y = estimate, colour = factor(n_year))) +
...
m4_max_fig_intercepts_3panel

# ------------------------------------------------------------
# 6g) Mean-salinity-only intercept figure (styled to pair with mid panel)
# ------------------------------------------------------------
m4_max_fig_intercepts_mean <- m4_max_int_summ %>%
  filter(sal_label == "Mean salinity") %>%
  mutate(
...
# Figure caption: For mean salinity conditions, the model shows how the predicted timing of peak oyster
# larval abundance differs among years.
# AKA (figure compares years to show whether peak larval abundance occurred earlier or later overall
# rather than showing how timing changes with temp)
# Salinity is fixed at the mean (0 SD) = typical salinity conditions relative to each bay
# Thick/thin vertical bars represent 50% and 90% Bayesian credible intervals around the mean predicted
# date of peak larval abundance
# (50% (thick) = 25th to 75th pct, 90% (thin) = 5th to 95th pct, points = mean predicted date for each year)
...
m4_max_fig_slopes_3panel <- ggplot(
  m4_max_slope_summ, aes(x = n_year, y = estimate, colour = factor(n_year), group = factor(n_year))) +
...
# of peak larval abundance changes with a 1°C increase in surface water temperature.
# AKA (shows whether warmer water is associated with earlier or later peak larval abundance in each 
...
# m4_max_alt_temp_levels <- tibble(
#   temp_level = factor(c("Cool", "Median", "Warm"),
#                       levels = c("Cool", "Median", "Warm")),
#   water_temp = c(m4_max_alt_ref$temp_cool,
#                  m4_max_alt_ref$temp_med,
#                  m4_max_alt_ref$temp_warm)
# )
# 
# # --- prediction grid (year sequence × temp levels), salinity fixed at mean ---
# m4_max_alt_newdat_year_temp <- crossing(
#   n_year = seq(min(m4_max_l_dat$n_year, na.rm = TRUE),
#                max(m4_max_l_dat$n_year, na.rm = TRUE),
#                length.out = 200),
#   temp_level = m4_max_alt_temp_levels$temp_level
# ) %>%
#   left_join(m4_max_alt_temp_levels, by = "temp_level") %>%
#   mutate(
#     salinity     = m4_max_alt_ref$sal_mean,
#     salinity.m   = salinity   - m4_max_alt_ref$sal_mean,
#     n_year.m     = n_year     - m4_max_alt_ref$n_year_mean,
#     water_temp.m = water_temp - m4_max_alt_ref$water_temp_mean
#   )
# 
# # --- posterior expected draws (population-level) ---
# m4_max_alt_draws <- m4_max_temp_time_sal %>%
#   add_epred_draws(newdata = m4_max_alt_newdat_year_temp, re_formula = NA) %>%
#   ungroup() %>%
#   rename(m4_max_alt_epred = .epred)
# 
# # --- summarise across draws (median + 80% interval) ---
# m4_max_alt_summ <- m4_max_alt_draws %>%
#   group_by(n_year, temp_level) %>%
#   median_qi(m4_max_alt_epred, .width = 0.80) %>%
#   ungroup()
# 
# # --- thin spaghetti: sample N draws total (not per temp; consistent across temps) ---
# m4_max_alt_n_keep <- 60
# 
# m4_max_alt_keep_draws <- m4_max_alt_draws %>%
#   distinct(.draw) %>%
#   slice_sample(n = min(m4_max_alt_n_keep, nrow(.)))
# 
# m4_max_alt_draws_thin <- m4_max_alt_draws %>%
#   semi_join(m4_max_alt_keep_draws, by = ".draw")
# 
# # --- plot ---
# m4_max_alt_fig_year3temp_spag_thin <- ggplot() +
#   geom_line(
#     data = m4_max_alt_draws_thin,
#     aes(
#       x = n_year,
#       y = m4_max_alt_epred,
...
#   )
# 
# m4_max_alt_fig_year3temp_spag_thin
# 
# 

# ============================================================
# m4 MAX: Ribbon fig + slopes + intercepts (matched to m2 START style)
# For: m4_max_temp_time_sal (data = m4_max_l_dat)
# - no custom functions
# - tidyverse + tidybayes
...
# ------------------------------------------------------------
# m4_max_01) References for centering + temp quantiles (UNCENTERED vars)
# ------------------------------------------------------------
m4_max_alt_ref <- m4_max_l_dat %>%
  ungroup() %>%
  summarise(
...
# ------------------------------------------------------------
# m4_max_02) Temperature level lookup table
# ------------------------------------------------------------
m4_max_alt_temp_levels <- tibble(
  temp_level = factor(c("Cool", "Median", "Warm"),
                      levels = c("Cool", "Median", "Warm")),
...
# ------------------------------------------------------------
# m4_max_03) Prediction grid (year sequence × temp levels), salinity fixed at mean
# ------------------------------------------------------------
m4_max_alt_newdat_year_temp <- crossing(
  n_year = seq(min(m4_max_l_dat$n_year, na.rm = TRUE),
               max(m4_max_l_dat$n_year, na.rm = TRUE),
...
# ------------------------------------------------------------
# m4_max_04) Posterior expected draws (population-level)
# ------------------------------------------------------------
m4_max_alt_draws <- m4_max_temp_time_sal %>%
  add_epred_draws(newdata = m4_max_alt_newdat_year_temp, re_formula = NA) %>%
  ungroup() %>%
...
# ------------------------------------------------------------
# m4_max_05) Summaries for ribbons:
#   90% interval for wide ribbon + 50% interval for inner ribbon + median line
# ------------------------------------------------------------
m4_max_alt_summ90 <- m4_max_alt_draws %>%
  group_by(n_year, temp_level) %>%
  median_qi(m4_max_alt_epred, .width = 0.90) %>%
  ungroup() %>%
  rename(
    m4_max_alt_med   = m4_max_alt_epred,
    m4_max_alt_low90 = .lower,
    m4_max_alt_up90  = .upper
  )

m4_max_alt_summ50 <- m4_max_alt_draws %>%
  group_by(n_year, temp_level) %>%
  median_qi(m4_max_alt_epred, .width = 0.50) %>%
  ungroup() %>%
  rename(
    m4_max_alt_low50 = .lower,
    m4_max_alt_up50  = .upper
  )

m4_max_alt_ribbon_dat <- m4_max_alt_summ90 %>%
  left_join(
    m4_max_alt_summ50 %>%
      select(n_year, temp_level, m4_max_alt_low50, m4_max_alt_up50),
    by = c("n_year", "temp_level")
  )

# ------------------------------------------------------------
# m4_max_06) Ribbon figure (year × temp; salinity held at mean)
# ------------------------------------------------------------
...
# geom_ribbon(aes(x = n_year, ymin = m4_max_alt_low50, ymax = m4_max_alt_up50, fill = temp_level), alpha  = 0.32, colour = NA) +
  ...
  scale_x_continuous(
    breaks = c(2012, 2014, 2016, 2018, 2020, 2022, 2024),
    labels = c("2012", "2014", "2016", "2018", "2020", "2022", "2024")) +
  coord_cartesian(ylim = quantile(m4_max_l_dat$julian_date, probs = c(0.01, 0.99), na.rm = TRUE)) +
  scale_y_continuous(
    breaks = scales::pretty_breaks(n = 6),
    labels = function(x) format(as.Date(x - 1, origin = "2000-01-01"), "%b %d")) +
  labs(
...
m4_max_alt_fig_year3temp_ribbons

# Figure caption: Across monitoring years (at mean salinity), the model shows how the predicted timing of
# peak oyster larval abundance varies under cool, median, and warm surface water temperature conditions.
# AKA (figure shows how the timing of the yearly peak in total larval abundance changes across years under diff 
...
# ============================================================
# m4 MAX: SLOPE FIGURE (matched to ribbon figure)
# - draw-by-draw slopes within temp_level
# - summarises across draws (mean + 50%/90% CrI)
# ============================================================

m4_max_slope_draws <- m4_max_alt_draws %>%
  group_by(.draw, temp_level) %>%
  summarise(
    slope = cov(n_year, m4_max_alt_epred) / var(n_year),
    .groups = "drop"
  )

m4_max_slope_summ <- m4_max_slope_draws %>%
  group_by(temp_level) %>%
  summarise(
    estimate = mean(slope),
    lower50  = quantile(slope, 0.25),
...
m4_max_fig_year3temp_slopes <- ggplot(
  m4_max_slope_summ, aes(x = temp_level, y = estimate, colour = temp_level)) +
...
m4_max_fig_year3temp_slopes

# Figure caption: Across monitoring years (at mean salinity), the model shows how the predicted timing of
# peak larval abundance changes through time under cool, median, and warm surface water temperature conditions.
# AKA (shows whether peak total larval abundance has shifted earlier or later over the monitoring period and
...
# dashed line = slope 0 = no long term change in timing of peak total larval abundance
# negative slope = earlier peak timing over time
# positive slope = later peak timing over time


# ============================================================
# m4 MAX: INTERCEPT FIGURE (matched to ribbons + slopes style)
...
m4_max_fig_year3temp_intercepts <- ggplot(
  m4_max_intercept_summ, aes(x = temp_level, y = estimate, colour = temp_level)) +
  geom_linerange(aes(ymin = lower90, ymax = upper90), linewidth = 0.9, alpha = 0.6) +
  geom_linerange(aes(ymin = lower50, ymax = upper50), linewidth = 2.2) +
  geom_point(size = 3) +
  scale_colour_manual(
...
# Figure caption: At a common reference monitoring year (at mean salinity), the model shows how the
# predicted timing of peak oyster larval abundance differs among cool, median, and warm surface water
# temperature conditions.
# AKA (figure compares baseline differences in when peak total larval abundance occurs under different
# temp conditions rather than showing changes through time)
# Salinity is fixed at the mean (0 SD) = typical salinity conditions relative to each bay
# The reference year represents the average monitoring year used for model centering
# Thick/thin vertical bars represent 50% and 90% Bayesian credible intervals around the mean predicted
# date of peak larval abundance
# (50% (thick) = 25th to 75th pct, 90% (thin) = 5th to 95th pct, points = mean predicted date)


# removing legends from graph e and f, then combining def
m4_max_fig_year3temp_intercepts <- m4_max_fig_year3temp_intercepts +
   guides(colour = "none", fill = "none") +
   theme(legend.position = "none")

m4_max_fig_year3temp_slopes <- m4_max_fig_year3temp_slopes +
  guides(colour = "none", fill = "none") +
  theme(legend.position = "none")

fig_7_combo_def <- ( m4_max_alt_fig_year3temp_ribbons +
                      m4_max_fig_year3temp_intercepts +
                      m4_max_fig_year3temp_slopes) +
  plot_layout(ncol = 3, guides = "collect") &
  theme(
...
# combining abc and def 

fig_7 <- (fig_7_combo_abc / fig_7_combo_def)
